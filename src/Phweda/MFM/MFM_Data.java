            ...
            if (new File(MFM.MFM_SETTINGS_DIR + MFM.MFM_MAME_FILE).exists()) {
                ...
                Thread loadMame = new Thread() {
                    ...
                    public void run() {
                        ...
                        mame = new Mame();
                        PersistUtils.retrieveJAXB(mame, MFM.MFM_SETTINGS_DIR + MFM.MFM_MAME_FILE, Mame.class);
                        if (MFM.isSystemDebug()) {
                            System.out.println("MAME load took: " +
                                    TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() - millis));
                        }
...
// NO!!!! NOTE Dependency on MAMEinfo which will not exist yet on first run to load resources. Moved to MFM.java
